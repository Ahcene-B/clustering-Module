...
def fc_layers(input, specs, wgt=None):
    [dimensions, activations, names] = specs
    for iw, (dimension, activation, name) in enumerate(zip(dimensions, activations, names)):    
        input = tf.layers.dense(inputs=input, 
            units=dimension, 
            activation=activation, 
            name=name, 
            kernel_initializer=( None if wgt is None else tf.constant_initializer( wgt[2*iw])  ),
            bias_initializer=( None if wgt is None else tf.constant_initializer( wgt[2*iw+1])  ),
            reuse=tf.AUTO_REUSE)
    return input

#def autoencoder(input, specs, wgt=None):
#    [dimensions, activations, names] = specs
...
#    else:
#        nw = int(len(wgt)/2)
#        embedding = fc_layers(input, [dimensions[:mid_ind], activations[:mid_ind], names[:mid_ind]], wgt[:nw])
#        output = fc_layers(embedding, [dimensions[mid_ind:], activations[mid_ind:], names[mid_ind:]],wgt[nw:])

#    return embedding, output
    

def encoder(input, specs, wgt=None):
    [dimensions, activations, names] = specs
    mid_ind = int(len(dimensions)/2)
...
def decoder(embedding, specs, wgt=None):
    [dimensions, activations, names] = specs
    mid_ind = int(len(dimensions)/2)
...
class DkmCompGraph(object):
    ...
    def __init__(self, ae_specs, n_clusters, val_lambda, wgt=None):
...
#        self.embedding, self.output = autoencoder(self.input, ae_specs, wgt)  # Get the auto-encoder's embedding and output

        self.embedding = encoder(self.input, ae_specs, wgt)  # Get the auto-encoder's embedding and output
        self.output    = decoder(self.embedding, ae_specs, wgt)  # Get the auto-encoder's embedding and output
